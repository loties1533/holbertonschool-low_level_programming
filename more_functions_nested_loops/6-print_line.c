 ...
 *
 * Description: La fonction affiche '_' ligne fois sur la même ligne.
 * Si ligne est inférieur ou égal à 0, elle affiche juste un retour à la ligne.
 */
void print_line(int ligne)
{
	int i;

	/* Si ligne est 0 ou négatif, on affiche seulement un retour à la ligne */
	if (ligne <= 0)
	{
		_putchar('\n');
		return;
